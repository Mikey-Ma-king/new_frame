...
__global__ void RowwiseMomentsCUDAKernel(
  ...
  __shared__ T_ACC s_sum[THREADS_PER_BLOCK];
  __shared__ T_ACC s_sumsq[THREADS_PER_BLOCK];

  T_ACC sum = 0;
  T_ACC sumsq = 0;
  for (int64_t j = threadIdx.x; j < N; j += blockDim.x) {
    const int64_t index = row * N + j;
    T_ACC v = static_cast<T_ACC>(dw_x[index]);
    sum += v;
    sumsq += v * v;
  }
  ...
  for (int offset = blockDim.x / 2; offset > 0; offset /= 2) {
    if (threadIdx.x < offset) {
      ...
      s_sumsq[threadIdx.x] += s_sumsq[threadIdx.x + offset];
    }
    __syncthreads();
  }
  ...
  if (threadIdx.x == 0) {
    T_ACC total = s_sum[0];
    T_ACC total2 = s_sumsq[0];
    T_ACC m = total / static_cast<T_ACC>(N);
    T_ACC var = total2 / static_cast<T_ACC>(N) - m * m;
    mean[row] = static_cast<T>(m);
    rstd[row] = static_cast<T>(c10::cuda::compat::rsqrt(var + static_cast<T_ACC>(eps)));
  }
}
